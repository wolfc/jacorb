...
//            org.omg.CosNaming.NamingContextExt nc = ss.get_naming();
    ...
    static boolean exec_command(){
		...
		//                lockset.print();
            ...
            } else if( cmd.length() >= 5 && cmd.substring(0,5).equals( "start" ) ) {
                if( ctrl != null ){
                    ...
                    try {
                     ...
                     System.out.println( "Transaction started with "+minute+"min timeout" );
                }
            } else { 
                if( ctrl == null ){
                    System.out.println("Error: Transaction not active");
                 ...
                 try {
                     ...
                     } else if( cmd.length() >=2 && cmd.substring(0,1).equals( "t" ) ) {
                         lock_mode mode = parse_mode( cmd.substring(1) );
                     ...
                     } else if( cmd.length() >=2 && cmd.substring(0,1).equals( "u" ) ) {
                         lock_mode mode = parse_mode( cmd.substring(1) );
                         lockset.unlock( ctrl.get_coordinator(), mode );
                         System.out.println( "Unlock complete" );
                     } else if( cmd.length() >=3 && cmd.substring(0,1).equals( "c" ) ) {
                         lock_mode mode = parse_mode( cmd.substring(1) );
                         lock_mode mode1;
                         if( mode.equals(lock_mode.intention_read) || mode.equals(lock_mode.intention_write) ){
                             mode1 = parse_mode( cmd.substring(3) );
                         } else {
                             mode1 = parse_mode( cmd.substring(2) );
                         }
                         lockset.change_mode( ctrl.get_coordinator(), mode, mode1 );
...
/*
    static Session get_session( org.omg.CORBA.ORB orb ) throws Exception {
        String line;
        BufferedReader in;
        Socket quote = new Socket("192.168.1.9", 9000);
        in = new BufferedReader(new InputStreamReader(quote.getInputStream()));
        line = in.readLine();
        ...
        org.omg.CORBA.Object o = orb.string_to_object(line);
        return SessionHelper.narrow(o);
    }
*/
};
